...
export function generatePDF(data: ReportData, type: string) {
  ...
  const doc = new jsPDF(
    type === "inventory" || type === "movements" ? "landscape" : "portrait"
  );
  ...
  const infoLines = [
    `Generado el: ${format(data.generatedAt, "dd/MM/yyyy HH:mm", {
      locale: es,
    })}`,
    data.dateRange.from && data.dateRange.to
      ? `Período: ${format(data.dateRange.from, "dd/MM/yyyy", {
          locale: es,
        })} - ${format(data.dateRange.to, "dd/MM/yyyy", { locale: es })}`
      : null,
  ...
  if (
    ...
    data.data.forEach((med: any) => {
      ...
      rows.push([
        ...
        med.supplier,
        med.category,
        format(med.expiryDate, "dd/MM/yyyy", { locale: es }),
        `$${med.price.toFixed(2)}`,
    ...
    data.data.forEach((mov: any) => {
      rows.push([
        ...
        format(mov.date, "dd/MM/yyyy", { locale: es }),
        mov.reason,
        mov.userName,
      ]);
    });
    ...
    data.data.forEach((alert: any) => {
      rows.push([
        ...
        alert.message,
        alert.severity,
        format(alert.date, "dd/MM/yyyy", { locale: es }),
      ]);
    });
    ...
    data.data.forEach((client: any) => {
      rows.push([
        ...
        client.lastPurchase
          ? format(client.lastPurchase, "dd/MM/yyyy", { locale: es })
          : "N/A",
      ]);
    });
...
export function generateExcel(data: ReportData, type: string) {
  ...
  excelData.push([
    "Generado el:",
    format(data.generatedAt, "dd/MM/yyyy HH:mm", { locale: es }),
  ]);

  if (data.dateRange.from && data.dateRange.to) {
    excelData.push([
      "Período:",
      `${format(data.dateRange.from, "dd/MM/yyyy", { locale: es })} - ${format(
        data.dateRange.to,
        "dd/MM/yyyy",
        { locale: es }
      )}`,
