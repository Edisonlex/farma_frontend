...
export function MovementDialog({ open, onOpenChange }: MovementDialogProps) {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      addMovement(values.type, String(values.medicationId), Number(values.quantity), values.reason);
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" min="1" value={field.value} onChange={(e) => field.onChange(Number(e.target.value))} placeholder="0" className={cn("h-11")} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Ej: LOT-2024-001" {...field} className={cn("h-11")} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          <div className="space-y-3">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="sr-only">Motivo del Movimiento</FormLabel>
                  <FormControl>
                    <Textarea placeholder="Describe el motivo del movimiento (compra, venta, ajuste, etc.)..." rows={3} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
          ...
          {form.watch("medicationId") && form.watch("quantity") && (
            <div className="p-4 bg-muted/50 rounded-lg border">
              ...
              <p className="text-sm text-muted-foreground">
                {form.watch("type") === "entrada" ? "➕ Entrada" : "➖ Salida"} de{" "}
